 ...
 * Test
 */
std::queue<int> Rbtree::TestGetInOrderResult() {
...
void Rbtree::TestInOrder(Rbnode* node, std::queue<int>& result) {
  if (node == mNil)
    return ;
...
std::queue<int> Rbtree::TestGetPostOrderResult() {
  std::queue<int> result;
...
void Rbtree::TestPostOrder(Rbnode* node, std::queue<int>& result) {
  if (node == mNil)
    return ;
...
int Rbtree::TestBlackDepth(void) {
  int before, now;
  std::stack<int> patrol, result;

  std::cout << "[BLACK DEPTH] ";
  if (mRootNode)
...
void Rbtree::TestPatrolBlackDepth(Rbnode* node, std::stack<int> patrol, std::stack<int>* result) {
  if (node == 0) {
    //		cout << patrol.size() << "/";
